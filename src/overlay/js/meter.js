// KawKaw engine — the single source of game logic, shared verbatim between the
// OBS overlay (browser global) and the relay bot (Node require). Pure logic:
// no DOM, no timers, no I/O. The caller feeds commands and drives one tick/sec.
//
...
(function (root, factory) {
  ...
  else root.KawKawEngine = factory();
  ...
  function createEngine(config = {}) {
    ...
    // Advance one tick. Returns true if state changed (caller broadcasts/renders).
    ...
    return { start, command, tick, getState, reset, config: cfg };
  }

  return { createEngine, DEFAULTS };
});
